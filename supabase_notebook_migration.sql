...
DROP POLICY IF EXISTS "Users can view their own notebook folders" ON notebook_folders;
DROP POLICY IF EXISTS "Users can insert notebook folders" ON notebook_folders;
DROP POLICY IF EXISTS "Users can update their own notebook folders" ON notebook_folders;
DROP POLICY IF EXISTS "Users can delete their own notebook folders" ON notebook_folders;
DROP POLICY IF EXISTS "Users can view their own notebook notes" ON notebook_notes;
DROP POLICY IF EXISTS "Users can insert notebook notes" ON notebook_notes;
DROP POLICY IF EXISTS "Users can update their own notebook notes" ON notebook_notes;
DROP POLICY IF EXISTS "Users can delete their own notebook notes" ON notebook_notes;
...
-- Create a default "General" folder for existing users
INSERT INTO notebook_folders (user_id, name, color)
SELECT id, 'General', '#3b82f6'
FROM users
WHERE NOT EXISTS (
  SELECT 1 FROM notebook_folders WHERE user_id = users.id
);