...
export default function CreateSessionModal({ open, onOpenChange, onSessionCreated }: CreateSessionModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isNaN(startingBalanceNum) || startingBalanceNum <= 0) {
        toast({
          title: "Validation Error",
          description: "Please enter a valid starting balance greater than 0.",
          variant: "destructive",
        });
        return;
      }
      ...
      const sessionData = {
        ...
        current_balance: startingBalanceNum, // Initialize current balance to starting balance
        pair: formData.pair,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[95vw] sm:max-w-[700px] max-h-[95vh] p-0 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border-slate-700/50 flex flex-col [&>button]:hidden" data-testid="modal-create-session">
        {/* Gradient Header */}
        <div className="relative bg-gradient-to-r from-cyan-500 via-blue-500 to-purple-600 p-6 pb-8 flex-shrink-0">
        ...
        {/* Form Content */}
        <div className="p-4 sm:p-8 bg-slate-900/95 backdrop-blur-sm flex-1 overflow-y-auto min-h-0">
